...
* Estimates are **static** today, not measurements: a recognised pattern is
  priced from the calibration table (`Calibrated`), and anything unclassified
  stays on the naive complexity heuristic (`Estimated`). Neither has been
  validated against profiling data, and runtime calibration plus the
  VeriSimDB-backed praxis loop are roadmap items. This is exactly why the
  confidence gate exists: heuristics advise; only measured or calibrated
  figures may ever block a merge.
