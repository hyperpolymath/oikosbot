...
Provide `GITHUB_APP_ID`, `GITHUB_PRIVATE_KEY`, and
`GITHUB_WEBHOOK_SECRET` through the deployment platform's secret store; do not
paste credential-shaped placeholder values into committed files. The optional
non-secret settings are `BOT_MODE` (`advisor`, `consultant`, or `regulator`)
and `PORT`.

For production, use a secrets manager. In Kubernetes, create an `oikos-bot`
Secret containing `GITHUB_APP_ID`, `GITHUB_PRIVATE_KEY`, and
`GITHUB_WEBHOOK_SECRET` outside version control, then reference the whole
Secret from the workload:
...
* Check that the private key has a PKCS#8 PEM header, rather than a PKCS#1 header
