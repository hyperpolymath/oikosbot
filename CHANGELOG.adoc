...
* fix(release): restore GHCR publication with a Rust 1.88 builder, publish
versioned image tags for release tags, and make the Action default to the
matching `+0.1.3+` image instead of the mutable `+latest+` tag.
